...
class UiMainWindow(QObject):
    ...
    def setupUi(self, mainwindowjai):
        mainwindowjai.setObjectName("mainwindow")
        ...
        self.connectRSButton.setEnabled(False)
        ...
        self.connectStageButton = QtWidgets.QPushButton(self.centralwidget)
        self.connectStageButton.setGeometry(QtCore.QRect(230, 330, 121, 24))
        self.connectStageButton.setObjectName("connectStageButton")
        self.connectStageButton.setDisabled(True)
        self.label_numberH = QtWidgets.QLabel(self.centralwidget)
        self.label_numberH.setGeometry(QtCore.QRect(230, 370, 121, 20))
        self.label_numberH.setObjectName("label_numberH")
        self.label_numberH.setDisabled(True)
        self.lineEdit_H = QtWidgets.QLineEdit(self.centralwidget)
        self.lineEdit_H.setDisabled(True)
        self.lineEdit_H.setGeometry(QtCore.QRect(230, 390, 100, 20))
        self.lineEdit_H.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight | QtCore.Qt.AlignmentFlag.AlignTrailing | QtCore.Qt.AlignmentFlag.AlignVCenter)
        ...
        self.connectStageButton.clicked.connect(self.setStageCommunication)
        self.lineEdit_H.textChanged.connect(self.changeHolonumber) #powiazany przycisk z funkcja
    ...
    def retranslateUi(self, mainwindowjai):
        ...
        mainwindowjai.setWindowTitle(_translate("mainwindow", "mainwindow"))
        ...
        self.lineEdit_V1.setText(_translate("mainwindow", "4"))
        self.pushButton_startTPS.setText(_translate("mainwindow", "Start TPS"))
        self.label_V1.setText(_translate("mainwindow", "Voltage 1"))
        self.lineEdit_Vstep.setText(_translate("mainwindow", "1.4"))
        self.label_Vstep.setText(_translate("mainwindow", "Voltage Step"))
        self.connectStageButton.setText(_translate("mainwindow", "Stage Connection"))
        self.label_numberH.setText(_translate("mainwindow", "Number of H:"))
        self.lineEdit_H.setText(_translate("mainwindow", "12"))
    ...
    def showSettings(self):
        ...
        if self.Doesexist("CH340") == 1:
            self.connectRSButton.setEnabled(True)
        if self.Doesexist("APT USB Device") == 1:
            self.connectStageButton.setEnabled(True)
    ...
    def setRSCommunication(self):
        self.serlista = list(serial.tools.list_ports.comports())
        portnumber = self.RSportnumber()
        self.serialRS = serial.Serial()
        self.serialRS.baudrate = 115200
        self.serialRS.port = self.serlista[portnumber].device
    ...
    def RSportnumber(self):
        self.serlista = list(serial.tools.list_ports.comports())
        RSname = "USB-SERIAL CH340"
        for n in self.serlista:
            indx = self.serlista.index(n)
            n = str(n)
            if RSname in n:
                return indx


    def Doesexist(self, device):
        self.serlista = list(serial.tools.list_ports.comports())
        for n in self.serlista:
            x = str(n)
            if device in x:
    ...
    def setStageCommunication(self):
        ...
        self.stage.identify()
        self.label_numberH.setEnabled(True)
        self.lineEdit_H.setEnabled(True)


    def changeHolonumber(self):
        self.holonumber = self.lineEdit_H.text()


    def calculateStageMoves(self):
        przesuwy = self.holonumber
        przesuwy = int(przesuwy)
        max0 = 414660
    ...
    def runAutoTPS(self):
        ...
        if (self.label_numberH.isEnabled()):
             TPSthread = threading.Thread(target=self.autocountTPSShot)
             TPSthread.start()
        else:
